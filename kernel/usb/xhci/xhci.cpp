...
namespace {
	...
	Error RegisterCommandRing(Ring* ring, MemMapRegister<CRCR_Bitmap>* crcr) {
		...
		crcr->Write(value);
		return USB_MAKE_ERROR(Error::kSuccess);
	}
	...
	Error ResetPort(Controller& xhc, Port& port) {
		...
		if (addressing_port != 0) {
			...
			if (port_phase != ConfigPhase::kNotConnected && port_phase != ConfigPhase::kWaitingAddressed) {
				return USB_MAKE_ERROR(Error::kInvalidPhase);
			}
	...
	Error AddressDevice(Controller& xhc, uint8_t port_id, uint8_t slot_id) {
		...
		if (dev == nullptr) {
			return USB_MAKE_ERROR(Error::kInvalidSlotID);
		}
		...
		InitializeEP0Context(
			*ep0_ctx, dev->AllocTransferRing(ep0_dci, 32), DetermineMaxPacketSizeForControlPipe(slot_ctx->bits.speed));
...
namespace usb::xhci {
	...
	Error ProcessEvent(Controller& xhc) {
		...
		Error err = USB_MAKE_ERROR(Error::kNotImplemented);
