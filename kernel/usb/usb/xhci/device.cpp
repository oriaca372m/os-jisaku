...
namespace usb::xhci {
	...
	Error Device::ControlIn(EndpointID ep_id, SetupData setup_data, void* buf, int len, ClassDriver* issuer) {
		...
		if (ep_id.Number() < 0 || 15 < ep_id.Number()) {
			return USB_MAKE_ERROR(Error::kInvalidEndpointNumber);
		}
		...
		if (tr == nullptr) {
			return USB_MAKE_ERROR(Error::kTransferRingNotSet);
		}
	...
	Error Device::InterruptOut(EndpointID ep_id, void* buf, int len) {
		...
		Log(kDebug,
			...
			len,
			this);
		return USB_MAKE_ERROR(Error::kNotImplemented);
	}
	...
	Error Device::OnTransferEventReceived(const TransferEventTRB& trb) {
		...
		if (trb.bits.completion_code != 1 /* Success */ && trb.bits.completion_code != 13 /* Short Packet */) {
			Log(kDebug, trb);
			return USB_MAKE_ERROR(Error::kTransferFailed);
		}
		Log(kDebug, trb);
		...
		if (!opt_setup_stage_trb) {
			...
			return USB_MAKE_ERROR(Error::kNoCorrespondingSetupStage);
		}
